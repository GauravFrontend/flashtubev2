...
export default function SignupForm() {
  ...
  const [identity, setIdentity] = useState(""); // To store username or email
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const isEmail = identity.includes("@"); // Check if it's an email or username
      const payload = isEmail
        ? { email: identity, password }
        : { username: identity, password };

      // Log the payload in the console
      console.log(payload);
